module SpanningTree (processEdgeList) where

import Data.Map as Map (Map, empty, findWithDefault, insert, insertWith, singleton)
import Data.Sequence as Seq (Seq (Empty, (:<|)), singleton, (|>))
import Data.Set as Set (Set, fromList, insert, notMember, singleton, toList)
...
bfsShortestPathEdgeListMap rootNode graphMap =
  bfsShortestPathEdgeListMap_ graphMap (Set.singleton rootNode) (Seq.singleton rootNode) (Map.singleton rootNode [rootNode])
  ...
  case focused_node_queue of
    ...
    current_focused_node :<| rest_focused_node_queue ->
      let
        ...
        new_shortest_path_edge_list_map = foldl (\new_map new_node -> Map.insert new_node (new_node : Map.findWithDefault [] current_focused_node new_map) new_map) shortest_path_edge_list_map new_neighbor_node_list
      in
